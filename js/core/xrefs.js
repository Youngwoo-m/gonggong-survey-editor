/* ============================================================
   ...
   돌아오지 않았다. 규정(3종)이 한 트리에 모였으니 그 일을 안으로 들인다.
   ...
   DOM 을 모른다.
   ============================================================ */
import * as M from "./model.js?v=20260907w";
import { isSelfCite, RE_JO_G } from "./objects.js?v=20260907w";
 ...
 * 우리가 담고 있는 규정(3종)을 가리키는 것만 본다 — 바깥 법령은 알 길이 없다.
 *
...
export const TERM_RULES = [
  ...
  },
  {
    canon: "기준점측량",
    variants: ["기준점 측량"],
    label: "이름씨 뒤의 '측량' 붙여쓰기",
    /* 앞말이 점ㆍ물인 것은 붙여 쓴다 — 기준점 · 삼각점 · 수준점 ·
       조절점 · 지하시설물. 앞가지가 붙은 것도 함께 걸린다
       (공공삼각점 측량 · 간접수준점 측량 · 지상기준점 측량).

       한때 이 규칙을 거꾸로 두어 267곳을 띄어 썼다. 국가공간정보
       표준용어집이 '지상 기준점' 으로 띄우는 것을 근거로 삼았는데,
       고시를 만드는 기관인 국토지리정보원의 「공간정보 용어사전」은
       기준점측량ㆍ도근점측량ㆍ지하시설물측량ㆍ지상기준점을 모두 붙여 쓰고
       띄어 쓴 표제어가 하나도 없다. 현행 고시 문언도 붙여 쓴다.
       소관 기관의 사전과 개정 대상 원문을 따라 되돌렸다.

       '지상 기준점' 처럼 측량이 붙지 아니한 이름씨는 이 규칙이 건드리지
       않는다 — 표준용어집의 표기를 그대로 둔다. */
    pattern: /([가-힣]{0,6}(?:삼각점|수준점|기준점|조절점|지하시설물))\s+측량/g,
    rewrite: (m) => `${m[1]}측량`,
    basis: "이름씨 뒤의 '측량' 을 붙여 쓴다 — 국토지리정보원 「공간정보 용어사전」의 표제어",
    std: { ko: "기준점측량 (基準點測量, control point surveying)",
           no: "국토지리정보원 공간정보 용어사전" },
    note: "'지상 기준점' 처럼 측량이 붙지 아니한 이름씨는 건드리지 않습니다.",
  },
  {
    canon: "무인비행장치",
    variants: ["무인항공기", "드론"],
...
/**
 * 규정(3종)이 같은 뜻을 다른 말로 부르는 자리를 모은다.
 ...
 * 규정 하나의 내용 지문.
 *
 * 판(버전)은 규정(3종)을 한꺼번에 담으므로, 어느 한 규정만 놓고 보면 여러 판이
