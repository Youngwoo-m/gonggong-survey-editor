...
아래 JSON 으로만 답하십시오.
{
  ...
  "reason": "개정 사유 한 문장 (개정 전후 비교표 비고란에 쓸 문장)",
...
각 조문의 개정 사유를 개정 전후 비교표 비고란에 쓸 한 문장으로 만들어 주십시오.

${ctx.changed}

아래 JSON 으로만 답하십시오.
{
  "items": [{"id": "노드 id 그대로", "reason": "개정 사유 한 문장"}]
}`,
    };
  },
  parse(j) {
...
export const compareRef = {
  key: "compareRef",
  name: "참조 규정과 견주기",
  hint: "② 창에 띄운 규정과 견주어 빠진 것·다른 것을 짚습니다.",
  needsNode: false,
  needsRef: true,
  build(ctx) {
    return {
