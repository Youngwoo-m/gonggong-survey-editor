...
class RowProto:
    ...
    def make_spec(self, row_no, cells):
        """칸마다 자리ㆍ폭ㆍ병합을 정해 <hp:tr> 한 줄을 만든다.

        cells 는 [{body, col, cs, rs, w, head}] 이다. 본문 속 표를 원본대로
        그리려면 폭과 병합을 칸마다 달리 주어야 한다 — 본 하나의 폭을
        되풀이해 쓰면 열이 많은 표가 본문폭을 넘는다."""
        out = ["<hp:tr>"]
        for c in cells:
            i = c.get("col", 0)
            ...
            head = tc[:tc.find(">") + 1]
            if c.get("head"):
                head = re.sub(r'header="\d"', 'header="1"', head)
            sl = re.search(r"<hp:subList\b[^>]*>", tc).group(0)
            out.append(
                head + sl + c["body"] + "</hp:subList>"
                + '<hp:cellAddr colAddr="%d" rowAddr="%d"/>' % (i, row_no)
                + '<hp:cellSpan colSpan="%d" rowSpan="%d"/>'
                % (c.get("cs", 1), c.get("rs", 1))
                + '<hp:cellSz width="%d" height="2000"/>' % c.get("w", 5000)
                + (re.search(r"<hp:cellMargin\b[^>]*/>", tc).group(0)
                   if re.search(r"<hp:cellMargin\b[^>]*/>", tc) else "")
                + "</hp:tc>")
        out.append("</hp:tr>")
        return "".join(out)
