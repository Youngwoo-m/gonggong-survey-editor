...
# ───────────────────────── 표를 문서 폭에 맞추기 ─────────────────────────
def fit_tables(hwpx_path, landscape=False, lr=5669):
    """표 폭을 본문폭에 맞춘다 — 칸 비율은 그대로 두고 함께 늘리거나 줄인다.

    landscape=True 면 쪽을 가로로 눕히고 좌우 여백을 lr 로 맞춘다
    (신구대조표 양식이 그렇다 — NARROWLY, 여백 5669, 본문폭 257mm).
    ...
    if landscape:
        xml = re.sub(r'(<hp:pagePr\b[^>]*?\blandscape=")[^"]*(")',
                     r"\g<1>NARROWLY\g<2>", xml)
        xml = re.sub(r'(<hp:margin\b[^>]*?\bleft=")\d+(")',
                     r"\g<1>" + str(lr) + r"\g<2>", xml)
        xml = re.sub(r'(<hp:margin\b[^>]*?\bright=")\d+(")',
                     r"\g<1>" + str(lr) + r"\g<2>", xml)
    ...
    # 눕힌 쪽에서는 긴 쪽(height)이 본문의 가로가 된다
    long_side = "height" if landscape else "width"
    target = (val(pp.group(0), long_side) - val(mg.group(0), "left")
    ...
    if not changed and not landscape:
        return []
